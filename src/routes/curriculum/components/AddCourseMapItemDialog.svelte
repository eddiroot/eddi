<script lang="ts">
	...
	export let defaultYearLevel: string = 'F';
	export let open: boolean = false;

	let title = '';
	let description = '';
	let yearLevel = defaultYearLevel;
	let termNumber = '1';
...
<Dialog bind:open>
	<DialogContent class="sm:max-w-[500px]">
		<DialogHeader>
			<DialogTitle>Add Course Map Item</DialogTitle>
			<DialogDescription>
				Create a new course map item for your curriculum planning.
			</DialogDescription>
		</DialogHeader>
			...
			use:enhance={() => {
				...
				return ({ result }) => {
					...
					if (result.type === 'success') {
						...
						const message = (result.data as any)?.message || 'Failed to create course map item';
