...
import { getNewsById, updateNews, getNewsCategories, attachResourceToNews, getNewsResources } from '$lib/server/db/service/news';
...
import { newsVisibilityEnum, newsStatusEnum } from '$lib/enums';
...
export const load: PageServerLoad = async ({ params, locals: { security } }) => {
    ...
    if (newsItem.news.authorId !== user.id && user.type !== 'schoolAdmin' && user.type !== 'systemAdmin') {
