...
export const actions: Actions = {
    default: async ({ request, locals: { security } }) => {
        ...
        try {
            ...
            console.log('📝 Processing content:', { content, length: content.length });
            ...
            const structuredContent = {
                blocks: content.split('\n\n').map((paragraph: string) => {
                    ...
                    console.log('🔍 Processing paragraph:', { trimmed, hasNewlines: trimmed.includes('\n') });
                    ...
                    if (trimmed.includes('\n')) {
                        ...
                        const result: Array<{ type: 'paragraph', content: string } | { type: 'list', items: string[] }> = [];
                        ...
                        if (result.length > 1) {
                            console.log('✅ Created mixed content blocks:', result);
                            return result; // This will be flattened below
                        }

                        // If we only found one block and it's a list, return it
                        if (result.length === 1 && result[0].type === 'list') {
                            console.log('✅ Created single list:', result[0]);
                            return result[0];
                        }
                    }

                    // Regular paragraph
                    console.log('📄 Created paragraph:', trimmed);
                    return {
                        type: 'paragraph',
                        content: trimmed
                    };
                }).filter(block => block !== null).flat() // Flatten arrays from mixed content
            };

            console.log('🏗️ Final structured content:', JSON.stringify(structuredContent, null, 2));
            ...
            if (action === 'publish') {
                console.log(`🔄 Redirecting to published news: /news?published=${news.id}`);
                throw redirect(303, `/news?published=${news.id}`);
            } else {
                console.log(`🔄 Redirecting to drafts page after saving draft: ${news.id}`);
