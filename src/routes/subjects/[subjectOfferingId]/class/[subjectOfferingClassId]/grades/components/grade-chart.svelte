<script lang="ts">
	...
	let {
		tasks = [] as Task[],
		view: viewProp = null as null | 'line' | 'bar',
		showControls = true
	} = $props();
	...
	const barData = tasks
		...
		.map((t) => ({
			...
			score: (t.studentGrade! / t.maxPoints) * 100,
			label: t.name
		}));
...
<div class="mb-0.5 flex items-center justify-between">
	<div class="text-muted-foreground text-sm">
		{currentView() === 'line'
			? 'Overall grade as results are released'
			: 'Each graded result (%) by date'}
	</div>
	{#if showControls}
...
<Chart.Container config={chartConfig} class="aspect-auto h-[230px] w-full">
	...
	{:else}
		{#key currentView()}
			<div in:fade={{ duration: 180 }} class="rise-bars h-full w-full">
				<BarChart
					data={barData}
					x="date"
					axis="x"
					...
					props={{
						...
						xAxis: {
							format: (d: Date) => d.toLocaleDateString('en-US', { month: 'short', day: '2-digit' })
						},
						yAxis: { format: (v: number) => `${v.toFixed(2)}%` }
					}}
				>
					{#snippet tooltip()}
						<Chart.Tooltip hideLabel nameKey="score">
							{#snippet formatter({ value, name, item })}
								{@const taskName =
									item && (item as any).payload
										? ((item as any).payload.label ?? (item as any).payload.datum?.label)
										: (item as any)?.label}
								<div class="w-full">
									{#if taskName}
										<div class="text-muted-foreground mb-0.5 truncate text-[11px]" title={taskName}>
											{taskName}
										</div>
									{/if}
									<div class="flex w-full items-center justify-between">
