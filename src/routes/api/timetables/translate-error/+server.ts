...
export async function POST({ request, locals: { security } }) {
	...
	try {
		...
		if (!Resource.GeminiApiKey.value) {
			throw error(500, 'Gemini API key not configured');
		}

		console.log(
			`🤖 [AI Translation] ${forceRetranslate ? 'Force re-translating' : 'Translating'} error for iteration ${iterationId}`
		);

		// Make the API call to Gemini
		const response = await fetch(
			`https://generativelanguage.googleapis.com/v1beta/models/${Resource.GeminiDefaultModel.value}:generateContent?key=${Resource.GeminiApiKey.value}`,
			{
				method: 'POST',
				headers: {
		...
		console.log(
			`✅ [AI Translation] Successfully translated error for iteration ${iterationId} (${forceRetranslate ? 'forced' : 'new'})`
		);
