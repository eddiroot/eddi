<script lang="ts">
	...
	let wholeSchool = $state(form?.formData?.wholeSchool ?? data.existingConfig?.wholeSchool ?? true);
	...
	let dateTimeRanges = $state<
		Record<string, { start: string; end: string; breaks: { start: string; end: string }[] }>
	>(
		form?.formData?.dateTimeRanges ??
			(typeof data.existingConfig?.dateTimeRanges === 'string'
				? JSON.parse(data.existingConfig.dateTimeRanges)
				: {})
	);
	...
	let duration = $state(form?.formData?.duration ?? data.existingConfig?.durationMinutes ?? 15);
	let autoAssign = $state(form?.formData?.autoAssign ?? data.existingConfig?.autoAssign ?? true);
...
<div class="space-y-8">
	...
	<form method="POST">
		{#if form?.error}
			<div
				class="bg-destructive/10 text-destructive border-destructive/20 mb-6 rounded-lg border p-4"
			>
				<h4 class="mb-2 font-medium">Please fix the following errors:</h4>
		...
		<input
			type="hidden"
			name="selectedDates"
			value={JSON.stringify(selectedDates.map(toIsoDate))}
		/>
			...
			<Card.Content class="space-y-6">
				...
				<div class="space-y-2">
					...
					<div class="flex items-center space-x-2">
						...
						<span class="text-muted-foreground text-sm">
							{wholeSchool
								? 'Yes, configure for all year levels'
								: 'No, select specific year levels'}
						</span>
					</div>
				</div>
				<!-- Year levels -->
				{#if !wholeSchool}
					<div class="space-y-2">
						...
						<div
							class="grid gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 {errors.yearLevels
								? 'border-destructive rounded-lg border p-2'
								: ''}"
						>
							{#each data.yearLevels as yl}
								<label class="flex items-center gap-2">
									<Checkbox
										value={yl.value}
										checked={yearLevels.includes(yl.value)}
					...
					<Popover.Root bind:open={calendarOpen}>
						<Popover.Trigger>
							{#snippet child({ props })}
								<Button
									{...props}
									variant="outline"
									class="flex-1 justify-between font-normal {errors.dates
										? 'border-destructive'
										: ''}"
						...
						<Popover.Content class="w-auto overflow-hidden p-0" align="start">
							<Calendar
								...
								isDateUnavailable={(date) => date.compare(currentDate) < 0}
							/>
							<div class="border-t p-3">
								{#if selectedDates.length > 0}
									<p class="text-muted-foreground mb-2 text-sm">
										Selected: {selectedDates.map(toIsoDate).join(', ')}
									</p>
								{/if}
								<div class="flex gap-2">
									...
									<Button
										...
										size="sm"
										class="flex-1"
										onclick={() => (calendarOpen = false)}>Done</Button
									>
								</div>
							</div>
						</Popover.Content>
								...
								<div class="rounded-lg border p-4">
									<div class="mb-3 flex items-center justify-between">
										...
										<Button
											...
											variant="outline"
											size="sm"
											onclick={() => removeDate(date)}>Remove Date</Button
										>
									</div>
									<div class="space-y-3">
										...
										<div class="space-y-2">
											<div class="flex items-center justify-between">
												...
												<Button
													...
													variant="outline"
													size="sm"
													onclick={() => addBreakForDate(dateStr)}>Add Break</Button
												>
											</div>
												...
												<div class="grid grid-cols-2 gap-3">
													...
													<div class="flex gap-2">
														...
														<Button
															...
															variant="outline"
															size="sm"
															onclick={() => removeBreakForDate(dateStr, breakIndex)}>Remove</Button
														>
													</div>
												</div>
											{/each}
				...
				<div class="space-y-2">
					...
					<Switch
						checked={slotGenerationMode === 'teacher'}
						onCheckedChange={(v) => (slotGenerationMode = v ? 'teacher' : 'class')}
					/>
					<span class="ml-2"
						>{slotGenerationMode === 'teacher' ? 'Teacher-based' : 'Class-based'}</span
					>
				</div>
				<!-- Interview duration -->
				<div class="space-y-2">
				...
				<div class="space-y-2">
					<div class="flex items-center space-x-2">
						<Checkbox
							checked={autoAssign}
							onCheckedChange={(v) => (autoAssign = v)}
							name="autoAssign"
						/>
