...
export const actions: Actions = {
	default: async ({ request, locals: { security } }) => {
		...
		if (autoAssign && selectedDates.length > 0) {
			...
			const teachers = await InterviewService.getTeachersByYearLevels(user.schoolId, wholeSchool ? null : yearLevels);
			...
			for (const teacher of teachers) {
				...
				if (slotGenerationMode === 'class') {
					...
					if (classes.length > 0) {
						...
						for (const cls of classes) {
							...
							if (classSlots.length > 0) {
								await InterviewService.createSlots(updatedConfig.id, teacher.id, cls.id.toString(), classSlots, duration, ['in_person']);
							}
						}
					}
				} else {
					...
					if (allSlots.length > 0) {
						await InterviewService.createSlots(updatedConfig.id, teacher.id, null, allSlots, duration, ['in_person']);
					}
				}
			}
		}
