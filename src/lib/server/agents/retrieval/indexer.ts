...
export class VectorIndexingService {
	...
	async initialize() {
		...
		console.log(
			`🔧 Initializing vector store for ${subjectIds.length} subjects: ${subjectIds.join(', ')}`
		);
	...
	private async indexAllDatabaseCurriculum() {
		...
		for (const subject of subjects) {
			...
			try {
				...
				const { keyKnowledge, keySkills, learningAreas, outcomes } =
					await curriculumService.getCurriculumDataForSubject(subject.id);
				...
				if (keyKnowledge.length > 0) {
					console.log(
						`      - Sample Key Knowledge: ${keyKnowledge[0].kk.description?.substring(0, 100)}...`
					);
				}
				if (keySkills.length > 0) {
					console.log(
						`      - Sample Key Skill: ${keySkills[0].ks.description?.substring(0, 100)}...`
					);
				}
				...
				console.log(
					`    ✅ Indexed ${formattedKeyKnowledge.length} key knowledge, ${formattedKeySkills.length} key skills`
				);
			} catch (error) {
	...
	private async indexAllCurriculumDataFiles() {
		...
		const dataPath = join(process.cwd(), 'data/curriculum-data');
		const files = readdirSync(dataPath).filter((f) => f !== 'pseudocode.json');
		...
		for (const file of files) {
			...
			try {
				...
				if (data.assessmentTasks?.length) {
					const formattedTasks = data.assessmentTasks.map((task) => ({
						...
						learningArea: this.formatLearningArea(
							typeof task.unit === 'string' ? parseInt(task.unit, 10) : task.unit,
							task.areaOfStudy
						)
					}));
				...
				if (data.outcomeAssessments?.length) {
					extraContent.push(
						...data.outcomeAssessments.map((item) => ({
	...
	private async indexPseudocode() {
		...
		try {
			...
			const mathSubjects = [specialistMath, mathMethods].filter(Boolean) as Array<{
				id: number;
				name: string;
			}>;
	...
	async testIndexing(subjectId: number, query: string = 'key concepts') {
		...
		results.forEach((result, index) => {
			console.log(
				`${index + 1}. [${result.source}/${result.metadata.type}] ${result.content.substring(0, 100)}...`
			);
		});
