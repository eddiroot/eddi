...
export class GuidanceVectorStore {
  private static instance: GuidanceVectorStore;
  private vectorStore: PGVectorStore | null = null;

  private constructor() {}

  static getInstance(): GuidanceVectorStore {
    if (!GuidanceVectorStore.instance) {
      GuidanceVectorStore.instance = new GuidanceVectorStore();
    }
    return GuidanceVectorStore.instance;
  }

  private async getVectorStore(): Promise<PGVectorStore> {
    if (!this.vectorStore) {
      this.vectorStore = await createVectorStore({
        collectionName: contentTypeEnum.guidance
      });
    }
...
export const guidanceVectorStore = GuidanceVectorStore.getInstance();