...
export class QuestionBankVectorStore {
  private static instance: QuestionBankVectorStore;
  private vectorStore: PGVectorStore | null = null;

  private constructor() {}

  static getInstance(): QuestionBankVectorStore {
    if (!QuestionBankVectorStore.instance) {
      QuestionBankVectorStore.instance = new QuestionBankVectorStore();
    }
    return QuestionBankVectorStore.instance;
  }

  private async getVectorStore(): Promise<PGVectorStore> {
    if (!this.vectorStore) {
      this.vectorStore = await createVectorStore({
        collectionName: contentTypeEnum.questionBank
      });
    }
...
export const questionBankVectorStore = QuestionBankVectorStore.getInstance();