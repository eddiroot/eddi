...
export class RubricCriteriaVectorStore {
  private static instance: RubricCriteriaVectorStore;
  private vectorStore: PGVectorStore | null = null;

  private constructor() {}

  static getInstance(): RubricCriteriaVectorStore {
    if (!RubricCriteriaVectorStore.instance) {
      RubricCriteriaVectorStore.instance = new RubricCriteriaVectorStore();
    }
    return RubricCriteriaVectorStore.instance;
  }

  private async getVectorStore(): Promise<PGVectorStore> {
    if (!this.vectorStore) {
      this.vectorStore = await createVectorStore({
        collectionName: contentTypeEnum.rubricCriteria
      });
    }
...
export const rubricCriteriaVectorStore = RubricCriteriaVectorStore.getInstance();