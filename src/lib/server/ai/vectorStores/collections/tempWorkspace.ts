...
export class TempWorkspaceVectorStore {
  private static instance: TempWorkspaceVectorStore;
  private vectorStore: PGVectorStore | null = null;

  private constructor() {}

  static getInstance(): TempWorkspaceVectorStore {
    if (!TempWorkspaceVectorStore.instance) {
      TempWorkspaceVectorStore.instance = new TempWorkspaceVectorStore();
    }
    return TempWorkspaceVectorStore.instance;
  }

  private async getVectorStore(): Promise<PGVectorStore> {
    if (!this.vectorStore) {
      this.vectorStore = await createVectorStore({
        collectionName: contentTypeEnum.tempWorkspace
      });
    }
...
export const tempWorkspaceVectorStore = TempWorkspaceVectorStore.getInstance();