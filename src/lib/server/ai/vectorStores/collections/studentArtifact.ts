...
export class StudentArtifactVectorStore {
  private static instance: StudentArtifactVectorStore;
  private vectorStore: PGVectorStore | null = null;

  private constructor() {}

  static getInstance(): StudentArtifactVectorStore {
    if (!StudentArtifactVectorStore.instance) {
      StudentArtifactVectorStore.instance = new StudentArtifactVectorStore();
    }
    return StudentArtifactVectorStore.instance;
  }

  private async getVectorStore(): Promise<PGVectorStore> {
    if (!this.vectorStore) {
      this.vectorStore = await createVectorStore({
        collectionName: contentTypeEnum.studentArtifact
      });
    }
...
export const studentArtifactVectorStore = StudentArtifactVectorStore.getInstance();