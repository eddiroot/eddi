...
export class FeedbackVectorStore {
  private static instance: FeedbackVectorStore;
  private vectorStore: PGVectorStore | null = null;

  private constructor() {}

  static getInstance(): FeedbackVectorStore {
    if (!FeedbackVectorStore.instance) {
      FeedbackVectorStore.instance = new FeedbackVectorStore();
    }
    return FeedbackVectorStore.instance;
  }

  private async getVectorStore(): Promise<PGVectorStore> {
    if (!this.vectorStore) {
      this.vectorStore = await createVectorStore({
        collectionName: contentTypeEnum.feedback
      });
    }
...
export const feedbackVectorStore = FeedbackVectorStore.getInstance();