...
export class MisconceptionVectorStore {
  private static instance: MisconceptionVectorStore;
  private vectorStore: PGVectorStore | null = null;

  private constructor() {}

  static getInstance(): MisconceptionVectorStore {
    if (!MisconceptionVectorStore.instance) {
      MisconceptionVectorStore.instance = new MisconceptionVectorStore();
    }
    return MisconceptionVectorStore.instance;
  }

  private async getVectorStore(): Promise<PGVectorStore> {
    if (!this.vectorStore) {
      this.vectorStore = await createVectorStore({
        collectionName: contentTypeEnum.misconception
      });
    }
...
export const misconceptionVectorStore = MisconceptionVectorStore.getInstance();